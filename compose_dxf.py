...
def gerar_imagem_plano(caminho_dxf, lista_arquivos):
    """
    Gera e salva uma imagem PNG apresentando o plano.
    O PNG será salvo no mesmo caminho do DXF, trocando extensão.
    """
    # Caminho completo para PNG
    png_path = caminho_dxf.replace('.dxf', '.png')
    # Converter cm->mm e calcular escala para px
...
def compor_dxf_com_base(lista_arquivos, caminho_saida):
    ...
    if first:
        ...
        doc_e = ezdxf.readfile(path)
        msp_e = doc_e.modelspace()
        cx, cy = calcular_centro(msp_e)
        dx, dy = COORDENADAS[1][0] - cx, COORDENADAS[1][1] - cy
        for ent in msp_e:
            try:
                ne = ent.copy()
                ne.translate(dx=dx, dy=dy, dz=0)
                msp.add_entity(ne)
            except:
                pass
    # blocos
    ...
    for nome, poses in grupos.items():
        ...
        msp_e = doc_e.modelspace()
        cx, cy = calcular_centro(msp_e)
        blk_name = f"BLK_{nome.replace('.', '_')}"
        blk = doc.blocks.new(name=blk_name)
        for ent in msp_e:
            try:
                ne = ent.copy()
                ne.translate(dx=-cx, dy=-cy, dz=0)
                blk.add_entity(ne)
            except:
                pass
        for pos in poses:
            msp.add_blockref(blk_name, insert=COORDENADAS[pos])
    # salvar DXF
